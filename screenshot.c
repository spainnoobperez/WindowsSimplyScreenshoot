...
DoSaveFile(HWND hwnd, LPTSTR szFileName) {
    ...
    if (CommDlgExtendedError() != CDERR_GENERALCODES)
        MessageBox(NULL, _T("Failed to save the file"), NULL, 0);
    return FALSE;
}
...
int WINAPI WinMain(HINSTANCE hInstance,
    ...
    if (CaptureScreen(pScrSht)) {
        ...
        if(ConvertDDBtoDIB(pScrSht)) {
            ...
            if(DoSaveFile(pScrSht->hSelf, szFileName)) {
                ...
                hadSaved = DoWriteFile(pScrSht, szFileName);
            }
        }
    }
    /* cleanup */
    ...
    if (pScrSht != NULL)
        HeapFree(GetProcessHeap(),
                 ...
                 pScrSht);
    if(hadSaved)
        MessageBox(NULL, TEXT("Screenshot has been saved"), TEXT("Complete!"), MB_ICONINFORMATION);
    return 0;
}
